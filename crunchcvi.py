...
def cvioutput( input, cvfxlimits, counterflowexcess, cvfxoptions, nullsignals, flowonoff, cvimode, C0, C1, C2, more, tdl_cals, opc_cal):
	...
	#NEEDS TO BE ADDED TO CALIBRATION SPACE
	...
	#Default Null Signals Originall Used in LabView (NEEDS TO BE CHANGED)
	#These Null signals allowed instruments to be ignored
	NullSignal = [0]*16
	NullSignal[7] = 1
	NullSignal[8] = 1
	NullSignal[9] = 1
	...
	cvfxtemp = cvfxoptions[5][:]
	cvfxtempsource = cvfxoptions[4][:]
	cvfxsw = cvfxoptions[0][:]
	cvfxmode = cvfxoptions[1][:]
	cvfxalt = cvfxoptions[2][:]
	cvfxdatatype = cvfxoptions[3][:]
							
	
	'''TEMPORARY'''
	cvfxdatatype[1] = 1		#Temporary overwrite to check values
	...
	for i in range(0,4):
		if cvfxtempsource[i] == 1 :
			...
			cvfxtemp[i] = cvfxtemp[i]#cvfxtempinput[i];
		if cvfxsw[i] == 0 :
			calibrated[i+6] = 0
			#cvfxaux[i] = 0
		else:
			if cvfxmode[i] == 1 :
				calibrated[i+6] = C0[i+6] + C1[i+6]*data[i+6] + C2[i+6]*data[i+6]**2
				#cvfxaux[i] = C0[i+6] + data[i+6]*C1[i+6] + data[i+6]*data[i+6]*C2[i+6]
			#elif cvfxmode[i] == 0 :
			else:
				calibrated[i+6] = cvfxalt[i] #USER ENTERED FLOW
				#cvfxaux[i] = cvfxalt[i] #user entry
			if cvfxdatatype[i] == 1 :
				calibrated[i+6] = calibrated[i+6]*(calibrated[10]/1013.23)*(294.26/(cvfxtemp[i]+273.15)) #calibrated[10] is cvpcn				
				#calibrated[i+6] = cvfxaux[i]*(calibrated[10]/1013.23)*(294.26/(cvfxtemp[i]+273.15)) #calibrated[10] is cvpcn				
	...
	for i in range(1,10):
		...
		summedflow = summedflow + calibrated[i]#calibrated[i+1]
	...
	#Ensure cvrad is a reasonable number?
	#Unecessary because loop limits possibilities
	...
	cvcfact_tdl=(cvtascc*math.pi*(calcoeffs[20]**2))/denom
	...
	#Not sure if log base 10 or log base e
	...
	opcc = (opc_data[0]*60)/(opc_data[1]*1000)
	opcco = opcc/cvcfact
